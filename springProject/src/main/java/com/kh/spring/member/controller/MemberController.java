...
public class MemberController {
	 ...
	 * 객체를 생성해서 주입해줌
	 * new 라는 키워드 없이 선언만 해준다. @Autowired어노테이션을 반드시 사용해야한다.
	 * 
	 */
	...
	*/
	...
	public String loginMember(Member m) {
		...
		// Member loginUser = new MemberServiceImpl.loginMember(m); (원래 하던 방식)
	...
	*/
	...
	/*
	 * 요청 처리 후 응답데이터를 담고 응답페이지로 포워딩 또는 url재요청을 하는 방법
	 * 
	...
	public String loginMember(Member m, Model model, HttpSession session) {
		...
		Member loginUser = memberService.loginMember(m);
		if(loginUser == null) { // 로그인 실패 => 에러문구를 requestScope에 담고 에러페이지로 포워딩
			model.addAttribute("erorrMsg", "로그인 실패");
	...
	*/
	...
	public ModelAndView loginMember(Member m, Model model, ModelAndView mv , HttpSession session) {
...
//		}
		...
		Member loginUser = memberService.loginMember(m); // 아이디로만 맴버객체 가져오기
		...
		if(loginUser == null || !bcryptPasswordEncoder.matches(m.getUserPwd(), loginUser.getUserPwd())) { // 로그인 실패 => 에러문구를 requestScope에 담고 에러페이지로 포워딩
			model.addAttribute("erorrMsg", "로그인 실패");
			mv.addObject("errorMsg", "로그인 실패");
	...
	public String insertMember(Member m, HttpSession session, Model model) {
		...
		m.setUserPwd(encPwd); // Member객체에 userPwd필드에 평문이아닌 암호문으로 변경
	...
	public String myPage(Member m, HttpSession session, ModelAndView mv) {
	...
	public String updateMember(Member m, HttpSession session, Model model) {
		...
		if(result > 0) {
			...
			// session영역에 loginUser라는 키값으로 덮어씌워야 한다.
